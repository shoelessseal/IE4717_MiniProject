...
<html lang="en">
    ...
    <body>
        ...
        $query_get = "SELECT image_PathLocation FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $img_path  = mysqli_fetch_array($query_result);

        $query_get = "SELECT video_PathLocation FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $vid_path  = mysqli_fetch_array($query_result);

        $query_get = "SELECT synopsis FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $synopsis  = mysqli_fetch_array($query_result);

        $query_get = "SELECT director FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $director  = mysqli_fetch_array($query_result);
        
        $query_get = "SELECT genre FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        ...
        $query_get = "SELECT movieDate FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $movieDate  = mysqli_fetch_array($query_result);

        $query_get = "SELECT movieLanguage FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $movieLanguage  = mysqli_fetch_array($query_result);

        $query_get = "SELECT duration FROM movies WHERE movieName= 'Spider-Man: No Way Home'";
        $query_result  = mysqli_query($db, $query_get);
        $duration  = mysqli_fetch_array($query_result);

        //TIMESLOTS FOR CAUSEWAY PT
        $CW_PT_TSS = array();
        $query_get = "SELECT timeSlotStart FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='CauseWay Point'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
            array_push($CW_PT_TSS, $result_value["timeSlotStart"]);
        }
        $CW_PT_TSE = array();
        $query_get = "SELECT timeSlotEnd FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='CauseWay Point'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
        ...
        $Orchard_TSS = array();
        $query_get = "SELECT timeSlotStart FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='Orchard Cineleisure'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
            array_push($Orchard_TSS, $result_value["timeSlotStart"]);
        }
        $Orchard_TSE = array();
        $query_get = "SELECT timeSlotEnd FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='Orchard Cineleisure'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
        ...
        //TIMESLOTS FOR JEM
        $JEM_TSS = array();
        $query_get = "SELECT timeSlotStart FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='JEM'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
            array_push($JEM_TSS, $result_value["timeSlotStart"]);
        }
        $JEM_TSE = array();
        $query_get = "SELECT timeSlotEnd FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='JEM'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
        ...
        $WestMall_TSS = array();
        $query_get = "SELECT timeSlotStart FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='WestMall'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
            array_push($WestMall_TSS, $result_value["timeSlotStart"]);
        }
        $WestMall_TSE = array();
        $query_get = "SELECT timeSlotEnd FROM timeSlots WHERE movieName= 'Spider-Man: No Way Home' AND locationName='WestMall'";
        $query_result  = mysqli_query($db, $query_get);
        foreach ($query_result as $result_value) {
            ...
            <div class="show-content">
                <div class="show-left-panel">
                    ...
                    <form action="seating.php" method="POST" onsubmit="">
                        ...
                        <div style="display: none;"><input type="text" name = "movieName" id = "movieName" value = "Spider-Man: No Way Home"></div>
